...
class FileTransferService {
  ...
  final String _receiveDirectory;
  ...
  Future<void> initialize() async {
    ...
    final receiveDir = io.Directory(_receiveDirectory);
  ...
  Future<void> _handleInit(
    ...
    final destDirAbs = _resolveOrCreateDestinationDirectory(destinationRel);
    if (destDirAbs == null) {
      _sendSignal(fileTransferInitAck, {
        'transferId': transferId,
        'accepted': false,
  ...
  String _defaultReceiveRelativePath() {
    final rel = p.relative(_receiveDirectory, from: _baseDirectory);
    return rel == '.' ? '' : rel;
  }
  ...
  String? _resolveOrCreateDestinationDirectory(String relativePath) {
    ...
    String? ensureDirectory(String relPath) {
      ...
      return _ensureDirectoryPath(absPath);
    }

    String? ensureAbsoluteDirectory(String absolutePath) {
      final normalized = p.normalize(absolutePath);
      return _ensureDirectoryPath(normalized);
    }

    final requested = ensureDirectory(normalizedRequested);
    if (requested != null) {
      return requested;
    }

    if (normalizedRequested != normalizedFallback) {
      final fallback = ensureDirectory(normalizedFallback);
      if (fallback != null) {
        return fallback;
      }
    }

    // Last-resort fallback: use receiveDirectory directly even if it is outside baseDirectory.
    final receiveAbs = ensureAbsoluteDirectory(_receiveDirectory);
    if (receiveAbs != null) {
      return receiveAbs;
    }

    // Final fallback: keep transfer alive by writing into base directory root.
    return ensureDirectory('');
  }

  String? _ensureDirectoryPath(String absPath) {
    try {
      final dir = io.Directory(absPath);
      if (!dir.existsSync()) {
